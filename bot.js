...
client.on("message", async message => {
  let pref = (await db.fetch(`prefix_${message.guild.id}`)) || "j!";
  let dil = await db.fetch(`dil_${message.guild.id}`);
  if (message.content === "<@!botid>") {

    if (dil == "tr") {
      message.channel.send(
        `Prefixim: \`${pref}\`\nEğer yardım istiyorsan; https://discord.gg/illegalizm`
      );
    } else {
      message.channel.send(
        `My prefix is: \`${pref}\`\nIf you want to get help; https://discord.gg/illegalizm`
      );
    }
  } else {
    return;
